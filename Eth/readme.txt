...
3) 기술의 신뢰성을 높이는 쪽에 주안점을 둠
4) 기술 증명을 위해 화폐에 적용 > 중앙은행으로부터 독립성을 이루는 것이 목적 > 비트코인 탄생

- 블록체인 2.0
1) 디지털 화폐를 넘어서기 위함
2) 자산(주식, 모기지, 부동산, 계약 등)들에 대한 생성과 거래가 가능케 됨 > 거래의 대상이 확장되어 가치가 높아짐
3) ethereum, Ripple, NXT, ...
4) 블록체인 1.0이 프로그래밍이 가능한 구조로 발전한 것임 
5) 이더리움
 > 2015년 7월 비탈릭 부텔린 1세대 블록체인(가상화폐) + 프로그래밍 기능 => 2세대 
6) 스마트 계약(스마트 컨트랙트) 등 DApp 같은 탈중앙화 어플리케이션(Decentralized Application)
 > 화폐의 관점 보다 명령어 등 프로그램 탑재 
 > 블록체인을 통해 작업 증명을 하는 과정을 계약 당사자 간의 확인으로 대체 가능

- 미래
> 블록체인 3.0 시대
> 화폐, 금융, 마켓, 정부, 헬스 등 전 분야로 확산 가능
> 각각의 디바이스를 블록체인으로 연결 하는 네트워크 구성이 가능 
